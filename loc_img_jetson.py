...
#     """
#     # 1) Load or reuse COLMAP reconstruction and prepare DB image mappings
...
#     # 2) Prepare the query image for feature extractors, keeping scale-back factors
...
#     # 3) Initialize device/extractors (reused if provided)
#     if device is None:
#         device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
#     # 4) Extract query features and scale keypoints back to the original pixel frame
#     feats_q = _extract_features_with_scaling(q_tensor_resized, extractors, device, (q_sx, q_sy))
#     t_qfeat = time.time() - t_qfeat_start

#     # 5) Retrieve top-K DB images with cached pHash (cheap)
#     preloaded = None
...
#     retrieved_ids = _retrieve_top_k_by_phash(scene_dir, query_image_path, db_id_and_paths, top_k_db, preloaded_ids_hashes=preloaded)
...
#     # 6) Decide whether to use registered 2D↔3D tracks or projection fallback
#     use_projection_fallback = (len(id_to_item) == 0)

#     # 7) Precompute a 3D subset for projection fallback (if needed)
...
#                 raise RuntimeError("points3D.ply has no vertices; cannot localize.")
...
#     # 8) For each retrieved DB image: load cached features, match, and gather 2D–3D
#     t_match_start = time.time()
#     for image_id in retrieved_ids:
#         img = recon.images[image_id]
#         image_path = os.path.join(images_dir, img.name)

#         # Load DB features from memory/cache (compute only as a last resort)
#         feats_d = None
...
#             cache_dir = prepared_scene["db_feature_cache_dir"]
#             cache_path = _feature_cache_path(cache_dir, image_id)
#             feats_d = _load_features_npz(cache_path)
#         if feats_d is None:
#             # Fallback: compute on-the-fly (should be rare once cache is built)
...
#         # Intersect modalities present in both query and DB, then match descriptors
...
#         # Use registered tracks if present; otherwise project global 3D into this DB view
...
#             # t_temp_elasped=time.time() -  t_temp
#             # print(f"Temp timer {t_temp_elasped}")
#             # exit()
...
#                 E = img.cam_from_world_matrix()  # 3x4
...
#     # 9) Concatenate correspondences across DB images
#     pts2d = np.concatenate(pts2d_all, axis=0)
#     pts3d = np.concatenate(pts3d_all, axis=0)

#     # 10) Estimate intrinsics from dominant retrieved camera (or median), then scale for query size
...
#     # 11) Solve PnP with RANSAC to estimate camera pose
...
#     # 12) Return pose, intrinsics, and debug timings/counters
#     debug = {
...
#         "t_matching": np.array([t_matching], dtype=np.float32),
